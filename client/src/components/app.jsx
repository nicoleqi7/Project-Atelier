...
class App extends React.Component {
  ...
  getProductInfo(id) {
    ...
    Promise.all(allPromises)
      .then((allPromisesData) => {
        var filteredRelatedProductsIDs = allPromisesData[2].data.filter(
          (id) => id !== this.state.productID
        );
  ...
  addOutfit() {
    ...
    if (!list.includes(this.state.productInfo.id.toString())) {
      localStorage.setItem(
        ...
        JSON.stringify(this.state.productInfo)
      );
  ...
  render() {
    ...
    return (
      <div  className="app">
        <h1 data-testid="header1" id="logo"> Good Deals Only </h1>
        <Navbar />
        <Overview
          productID={productID}
        ...
        Object.keys(reviews).length === 0 ? null : (
